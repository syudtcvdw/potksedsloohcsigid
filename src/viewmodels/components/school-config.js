var vm = function (params) {
	...
	vm.sessions = ko.observableArray(sessions)
	vm.subSessions = ko.observableArray(subSessions)
	vm.selectedSession = ko.observable()
	vm.selectedSubSession = ko.observable()
	...
	vm.setSession = () => {
		/**
		 *  NOTE: Just running these codes in my head.
		 */
		if ( _anyEmpty(vm.selectedSession(), vm.selectedSubSession()) ) 
			return VM.notify('Please provide necessary information as appropriate.', 'warn'), null
		// set the session
		sockets.emit('set session', { 'session': vm.selectedSession(), 'sub-session': vm.selectedSubSession() },data => {
			if ( !data.status )
				return VM.notify('Could not set the session.'), null
			else {
				VM.notify('Session successfully set!')
			}
		})
	}
